...
def display_snake(snake_position, display):
    #loop over each position item to create snake of desired length.   
    for position in snake_position:
        #displaying one box of snake body
        pygame.draw.rect(display, (255, 0, 0), pygame.Rect(position[0], position[1], 10, 10))


def display_apple(apple_position, display):
    # display apple as red green box.
...
def generate_snake(snake_start, snake_position, apple_position, button_direction, score):
    ...
    if snake_start == apple_position:
        #apple position will be updated
        apple_position, score = collision_with_apple(apple_position, score)
        # and snake will be elongated
        snake_position.insert(0, list(snake_start))

    else:
        #Snake elongated 
        snake_position.insert(0, list(snake_start))
        #snake reduced by one step.
        snake_position.pop()

    return snake_position, apple_position, score

# what happens when snake eats the apple
def collision_with_apple(apple_position, score):
    
    apple_position = [random.randrange(1, 50) * 10, random.randrange(1, 50) * 10]  #Find another position for apple randomly
    score += 1 #increment score. 
    return apple_position, score

# what happens when snake touches the boundaries
...
def angle_with_apple(snake_position, apple_position): #uses previous snake and apple position to set angle of snake with apple
    apple_direction_vector = np.array(apple_position) - np.array(snake_position[0]) # difference of apple position and snake position
    snake_direction_vector = np.array(snake_position[0]) - np.array(snake_position[1]) # difference of snake position[0] and snake position [1]
    # calculating norm of apple and snake vectors.
    norm_of_apple_direction_vector = np.linalg.norm(apple_direction_vector)
    norm_of_snake_direction_vector = np.linalg.norm(snake_direction_vector)
    
    #adjusting 0 norms exceptions
    if norm_of_apple_direction_vector == 0:
        norm_of_apple_direction_vector = 10
    if norm_of_snake_direction_vector == 0:
        norm_of_snake_direction_vector = 10
 
    #normalizing the vecors
...
# function to run the snake until it crashes itself.
